							...
							<div class="col-md-12">
								<div class="form-floating mb-3">
									<select class="form-control @error('crop_type') is-invalid @enderror" id="crop_type"
										name="crop_type" aria-label="Crop Type" data-live-search="true" title="Choose Crop Type" required>
										@foreach (config('constants.crop_types') as $crop_type)
										<option value="{{ $crop_type }}" {{ old('crop_type')|| $highValueCrop->crop_type==$crop_type ? 'selected' : '' }}>
											{{ $crop_type }}
										</option>
										@endforeach
									</select>
